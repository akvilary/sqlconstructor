...
def test_set_variables_by_calling_container_after_building_query(
    simple_query_dict_with_placeholders, simple_query_sql
):
...
@pytest.mark.SqlSection
@pytest.mark.SqlContainer
def test_set_variables_by_vars_attribute_after_building_query(simple_query_dict_with_placeholders, simple_query_sql):
    q = SqlQuery(simple_query_dict_with_placeholders)
    container = q()
